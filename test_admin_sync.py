#!/usr/bin/env python3
"""
Test the admin donations page sync functionality
"""
...
def test_admin_sync():
    """Test admin donations page sync functionality"""
    print("🧪 Testing Admin Donations Page Sync")
    print("=" * 50)
    ...
    with app.app_context():
        ...
        try:
            ...
            if all_worksheet_data:
                ...
                # Show sample data for each tab
                for purpose_name, worksheet_data in all_worksheet_data.items():
                    print(f"\n📋 {purpose_name} Tab:")
                    print(f"   Total Donations: {worksheet_data['summary']['total_donations']}")
                    print(f"   Total Amount: ₹{worksheet_data['summary']['total_amount']:.2f}")
                    print(f"   Verified: {worksheet_data['summary']['verified_donations']}")
                    print(f"   Pending: {worksheet_data['summary']['pending_donations']}")
                    print(f"   Average: ₹{worksheet_data['summary']['average_donation']:.2f}")
                    
                    # Show first few donations
                    print(f"   Sample donations:")
                    for i, donation in enumerate(worksheet_data['donations'][:3]):
                        print(f"      {i+1}. {donation.donor_name} - ₹{donation.amount} (ID: {donation.donor_id})")
            else:
